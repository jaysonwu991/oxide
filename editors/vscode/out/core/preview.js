...
exports.canonicalTool = canonicalTool;
exports.toolDiff = toolDiff;
const CONTEXT = 3;
...
function stripLinePrefixes(text) {
    ...
    for (const line of lines) {
        ...
        if (!prefix || prefix.number <= previous)
            return null;
...
function stripAllLinePrefixes(text) {
    return text
        .split("\n")
...
function locate(base, old) {
    ...
    if (exact.length === 1)
        return { start: exact[0], end: exact[0] + old.length };
    if (exact.length > 1)
        return "ambiguous";
    const baseLines = sourceLines(base);
    const oldLines = sourceLines(old);
    if (oldLines.length === 0 || oldLines.length > baseLines.length)
        return null;
    ...
    for (let i = 0; i + width <= baseLines.length; i += 1) {
        ...
        for (let k = 0; k < width; k += 1) {
            if (baseLines[i + k].text.replace(/\s+$/, "") !== oldLines[k].text.replace(/\s+$/, "")) {
                matches = false;
                break;
            }
        }
...
function toolDiff(name, args, readFile) {
