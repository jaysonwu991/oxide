...
- `src/tui/` — ratatui + crossterm interface; renders conversation lines incrementally (only changed items are re-wrapped), shows the active mode and reasoning level and cycles them with Shift+Tab / Ctrl+R, and routes Enter while busy into the steering queue.
