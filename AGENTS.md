...
- `src/tui/` — ratatui + crossterm interface; renders conversation lines incrementally (only changed items are re-wrapped), shows the active mode and reasoning level and cycles them with Shift+Tab / Ctrl+R, and routes Enter while busy into the steering queue. It launches even without a provider key; `/connect` opens a provider/key dialog (bracketed paste enabled) and applies the credential to the running config.
