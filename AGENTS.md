...
- `src/config.rs` — `Config` struct; loads `config.json` from the oxide config dir (`dirs::config_dir()/oxide`, e.g. `~/.config/oxide` on Linux, `~/Library/Application Support/oxide` on macOS) plus `OXIDE_*` / provider env overrides. Provider presets cover OpenAI/GPT, DeepSeek and Anthropic (`OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`). Defines the `Mode` enum (`build` / `plan` / `auto-edit`, set via `--mode`, `OXIDE_MODE`, or config) and the `Reasoning` enum (`auto` / `off` / `low` / `medium` / `high`, set via `--reasoning`, `OXIDE_REASONING`, or config; `auto` resolves per model via `effective_reasoning`), composes the system prompt (adding plan-mode instructions), resolves `/commands` (including `agent`/`subtask` routing), and loads the context-pruning config from `.oxide/dcp.json`.
- `src/auth.rs` — `auth login` / `list` / `logout`: stores provider API keys in `auth.json` in the oxide config dir (mode 0600); keys are resolved after env vars and before the config file. `connect` (used by `auth login` and the TUI `/connect` command) also writes the active provider to `config.json` so the next launch uses it.
...
- `src/llm/` — model client. `client.rs` dispatches by provider: OpenAI-compatible chat completions (OpenAI, DeepSeek, custom) and the Anthropic Messages API (`anthropic.rs`, converting internal messages to content blocks and parsing its SSE events). Applies the configured reasoning level: OpenAI `reasoning_effort` and Anthropic extended thinking (`budget_tokens`), with Anthropic thinking blocks replayed on later turns.
...
- `src/tools.rs` — tool specs and execution (`read_file`, `write_file`, `list_dir`, `bash`, `glob`, `grep`, `patch`, `webfetch`) plus connected MCP tools. `execute(call, cwd, &McpRegistry, &Progress)` returns a `ToolOutput` (text plus optional image/PDF media parts and a `terminate` flag; `read_file` attaches images/PDFs); `bash` streams stdout/stderr through `Progress`. Output is capped centrally in `execute` by lines (`MAX_OUTPUT_LINES`) and bytes (`MAX_OUTPUT_BYTES`): `bash` keeps its tail so the exit code survives, other tools keep the head, and dropped content is written under `truncated/` in the oxide config dir (`OXIDE_TRUNCATION_DIR`, retained 7 days) with a pointer in the result. `read_file` trims lines at `MAX_LINE_LEN` and pages via `offset`/`limit`.
- `src/tui/` — ratatui + crossterm interface; renders conversation lines incrementally (only changed items are re-wrapped), hides tool-output bodies by default (Ctrl+O toggles), truncates action lines to the terminal width, shows the active mode and reasoning level and cycles them with Shift+Tab / Ctrl+R, and routes Enter while busy into the steering queue. The input box grows with the message (up to 8 rows) and the status line shows elapsed time while busy. It launches even without a provider key; `/connect` opens a provider/key dialog (bracketed paste enabled) and applies the credential to the running config, and `/help` lists built-in and discovered commands/agents/skills.
...
- Built-in file/shell tools and connected MCP tools are registered in `tools::specs(&McpRegistry)` and dispatched in `tools::execute(call, cwd, &McpRegistry, &Progress)`. The agent-level `task`, `skill`, `memory`, `diagnostics` and `compress` tools are defined and dispatched in `src/agent.rs`.
