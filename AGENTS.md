...
An optimized AI coding agent CLI written in Rust, inspired by opencode.

## Layout

- `src/main.rs` — CLI entry (clap). Non-interactive `-p/--print` mode and TUI dispatch.
- `src/config.rs` — `Config` struct; loads `~/.config/oxide/config.json` plus `OXIDE_*` / provider env overrides. Provider presets cover OpenAI/GPT, DeepSeek and Anthropic (`OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`). Composes the system prompt and expands `/commands`.
- `src/auth.rs` — `auth login` / `list` / `logout`: stores provider API keys in `~/.config/oxide/auth.json` (mode 0600); keys are resolved after env vars and before the config file.
- `src/ecosystem/` — discovers and loads the Claude Code + OpenCode ecosystem from project and global scope: rules, memory (`CLAUDE.md`), commands, agents/subagents, skills (`SKILL.md`), MCP servers (`opencode.json` / `.mcp.json`) and plugins.
...
- `src/memory.rs` — persistent cross-session memory store (project + user scope) under `~/.config/oxide/memory/`, with dependency-free tf-idf search.
- `src/session.rs` — durable append-only session log (JSONL) under `~/.config/oxide/sessions/<project>/`; model history is reconstructed from it and resumed via `-c/--continue` or `--resume <id>`.
- `src/snapshots.rs` — shadow-git snapshots (bare repo under `~/.config/oxide/snapshots/<project>/`) committed after each agent step; `/undo` and `/redo` in the TUI restore file changes.
- `src/permission.rs` — parses `permission` rules (top-level or per-agent) into allow/ask/deny and decides per tool + subject; `ask` prompts in the TUI unless `auto_approve` is set.
